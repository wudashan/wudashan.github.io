...
贪心算法采用的是这样一种思想：尽量先走出路比较少的格子（相同出路的格子只走一次），这样，后面的步骤可选择的余地就大，最优解的概率也就大的多。
...
由于贪心算法并没有遍历所有路径，而是先走出路少的格子，相同的出路的格子只走一次，所以时间上快很多，但是其结果却不一定是最优路径！这是因为，寻找哈密顿路径是一个典型的NPC问题，如果想要得到最优解则不能在多项式时间内解出答案。
