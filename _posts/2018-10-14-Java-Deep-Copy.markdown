...
| 深拷贝方法 | 优点 | 缺点
|----|------|---- 
| 构造函数 | 1. 简单易理解 <br> 2. 不需要引入第三方包 |  1. 可用性差，每次新增成员变量都需要新增新的拷贝构造函数
| 重载clone()方法 |  1. 不需要引入第三方包  |  1. 可用性较差，每次新增成员变量可能需要修改clone()方法 <br> 2. 拷贝类（包括其成员变量）需要实现Cloneable接口
...
|Jackson序列化 | 1. 可用性强，新增成员变量不需要修改拷贝方法 | 1. 需要引入Jackson第三方JAR包 <br> 2. 拷贝类（包括其成员变量）需要实现默认的无参构造函数
